...
- Delegate non-trivial work, but size the job to the tier and know what you are starting: the workers you call can call workers of their own, so one delegation can expand into a tree several levels deep, each level costing a full model turn before any work happens. Use general or custom for work that needs judgment, the -2wide variants for a large divisible list, -wide for a moderate batch, -deep for one thread, and the explore family for pure search. Working inline is always a legitimate choice.
