...
const r2 = await tools.mcp_chrome_devtools.evaluate_script({ function: "function() { if (!globalThis.YAML) return 'NO YAML'; var yaml = '# Customer config\\ncustomer_number: 123 # account id\\ncustomer_long_name: Test Corp\\n# end\\n'; var doc = YAML.parseDocument(yaml); doc.set('customer_number', 456); var result = doc.toString(); return JSON.stringify({ output: result, commentsPreserved: result.indexOf('# Customer config') > -1 && result.indexOf('# account id') > -1 && result.indexOf('# end') > -1 }); }" });
